...
const ICONS = {
    ...
    close: `<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" x2="6" y1="6" y2="18"/><line x1="6" x2="18" y1="6" y2="18"/></svg>`,
    spinner: `<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="spinning"><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>`,
...
function showToast(message, type = 'info') {
    ...
    const icon = type === 'error' ? ICONS.error : type === 'success' ? ICONS.success : ICONS.info;
...
function handleFileSelect(file) {
    ...
    <span>${ICONS.file} ${file.name} (${formatFileSize(file.size)})</span>
    <button class="btn btn-outline btn-sm" onclick="clearFile()">${ICONS.close} Remove</button>
  `;
    updateProcessButton();
}
...
function updateProcessButton() {
    ...
    if (state.isProcessing) {
        ...
        elements.processBtn.innerHTML = `${ICONS.spinner} Processing...`;
        return;
    }

    elements.processBtn.innerHTML = `${ICONS.play} Process Cases`;
