SNAP-AI Layer 2: CIE – Complication Inference Engine (v1.11 — Episode-Aware, Severity-Guarded, CD-I Sweep, CCI Self-Check)
...
ANTI-FALSE-POSITIVE RULES FOR CD I (v1.10 — BINDING):
...
BINDING EPISODE SPLITTING RULE — "CAPILLARY LEAK" UMBRELLA (v1.11 — CRITICAL):
...
SPLITTING RULE: Split into EXACTLY the following episode types (no more, no less):
  ...
  • Hypervoläme Hyponatriämie → SEPARATE episode, Grade II
    (when Albumin-Substitution is documented in the same patient context,
    albumin is a NON-EXEMPT blood product → Grade II for this episode)

CRITICAL ANTI-OVER-SPLITTING RULE (v1.11 — BINDING):
...
BINDING EXAMPLE (Case 14 pattern — v1.11):
  ...
  CORRECT extraction (EXACTLY 4 episodes — no more):
  ...
  WRONG extraction #1 (merged into 1-2 episodes):
    ...
    → CCI would be drastically underestimated.

  WRONG extraction #2 (over-split into 5 episodes):
    ✗ Adding a 5th episode "Anasarka/capillary leak" → Grade II
    → This DOUBLE-COUNTS the albumin treatment (already in hyponatremia episode).
    → Anasarka is the underlying cause, NOT a separate complication.
    → CCI would be overestimated: [IIIa, IIIa, II, I, II] = 48.2 (WRONG)
