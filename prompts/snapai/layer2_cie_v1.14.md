SNAP-AI Layer 2: CIE – Complication Inference Engine (v1.14 — Clinical Reasoning Pipeline)
...
ASSESSMENT FINDINGS (NOT complications — do NOT extract):
- Nutritional assessments (e.g., "Mangelernährung", "significant energy and protein malnutrition") WITHOUT specific pharmacological in-hospital treatment (Grade II+) → NOT a complication. Dietary support/nutritional counseling alone does NOT qualify.
- Radiologic findings WITHOUT clinical consequence: if the text explicitly states that NO treatment was given because parameters were regressive/stable, the finding is an observation, NOT a deviation.
- Incidental lab values without clinical action

CONTRASTIVE EXAMPLE — Malnutrition (v1.14):
  Text: "Significant energy and protein malnutrition. Nutritional support via dietary progression."
  ✗ WRONG: Extract as CD Grade I complication
  ✓ CORRECT: REJECT. Malnutrition is an assessment finding. Dietary progression is routine care, not a deviation.

CONTRASTIVE EXAMPLE — Radiologic finding without treatment (v1.14):
  Text: "Lower-lobe bilateral lung consolidations were noted, but because infection parameters were regressive, no antibiotic therapy was initiated."
  ✗ WRONG: Extract as CD Grade I (radiologic finding = deviation)
  ✓ CORRECT: REJECT. The text EXPLICITLY states no treatment was needed because parameters were regressive. A radiologic finding that is explicitly managed conservatively with no treatment and no clinical impact is NOT a deviation — it is an observation.
  → KEY RULE: If the text says "no treatment was given because [parameters OK / regressive / stable]", this is NOT a complication.
...
CONTRASTIVE EXAMPLE — Diarrhea with Loperamide only (v1.14):
  Text: "The patient had 3-4 liquid stools per day, treated with Loperamide as needed."
  ✗ WRONG: Extract as separate CD Grade I complication
  ✓ CORRECT: REJECT. Loperamide is a CD-I EXEMPT drug. Diarrhea treated ONLY with Loperamide without additional workup or escalation is routine symptom management, NOT a separate complication episode. Only extract diarrhea if it leads to non-exempt treatment (antibiotics, IV fluids for dehydration, etc.) or triggers a workup revealing a complication.

============================================================
STEP 3 — SCOPE INTERPRETATION (CRITICAL)
============================================================
...
- BUT: vague observations, routine recovery, assessment findings, and radiologic observations without clinical impact are NOT Grade I
...
Invalid — do NOT extract as CD I:
...
- "Mangelernährung" / malnutrition without specific pharmacological treatment
- Diarrhea managed ONLY with Loperamide (exempt drug, routine symptom management)
- Radiologic findings explicitly managed conservatively with no treatment and no clinical consequence
...
INTERVENTIONAL PROCEDURE CHECK (v1.14 — MANDATORY):
Scan the entire text for evidence of ANY interventional procedure performed postoperatively:
  - "punktiert" / "Punktion" / "puncture" → likely Grade IIIa
  - "Drainage" (new placement, NOT routine removal) → likely Grade IIIa
  - "Aszites ... punktiert" / "ascites ... drained" → Grade IIIa
  - "Pleuraerguss ... punktiert" / "pleural effusion ... puncture" → Grade IIIa
  - "CT-gesteuerte Drainage" → Grade IIIa
  - "Reoperation" / "Revision" → Grade IIIb
If found, this MUST appear as a complication episode. Do NOT miss interventional procedures.
...
GRADE DEFINITIONS:
  ...
  CD IIIa: Intervention WITHOUT general anesthesia (IR procedures, endoscopy,
           ascites drainage, pleural puncture, CT-guided drainage, percutaneous puncture).
...
BEDSIDE vs PROCEDURE ROOM:
  ...
  Procedure room (Grade IIIa): ascites drainage/puncture, pleural puncture, CT-guided drainage

ICU RULE: ICU admission ALONE ≠ Grade IV. Grade IV requires EXPLICIT organ dysfunction.

SPECIFIC GRADING PATTERNS (BINDING — v1.14):
  "adaptierte Darmstimulation" + named ileus diagnosis → Grade II (implies prokinetics)
       This is ALWAYS Grade II when the text names the ileus as a diagnosis. Do NOT downgrade to I.
  "Magensonde" alone without prokinetics → Grade I
  Hyponatremia + albumin given ANYWHERE during same admission → Grade II
       Even if albumin was given for a different indication, the presence of albumin = non-exempt blood product → Grade II.
  ...
  Percutaneous puncture of ascites → Grade IIIa (NOT Grade I or II)
  Percutaneous puncture of pleural effusion → Grade IIIa
...
CONTRASTIVE EXAMPLES:
  ...
  ✓ CORRECT: "Paralytic ileus → Grade II" (adapted stimulation implies prokinetics = non-exempt)

  ✗ WRONG: "Urinary retention → Grade I" when Tamsulosin/Pradif was given
  ✓ CORRECT: "Urinary retention → Grade II" (Tamsulosin = non-exempt)

  ✗ WRONG: "Ascites punctured percutaneously → Grade II"
  ✓ CORRECT: "Ascites punctured percutaneously → Grade IIIa" (percutaneous puncture = intervention without GA)
...
□ INTERVENTIONS: Did I check for percutaneous punctures / drainage? (MUST be IIIa)
...
□ ASSESSMENT: No malnutrition/radiologic observations without treatment extracted
...
5. "Did I miss any percutaneous puncture or drainage?" → If yes, ADD as IIIa.
6. "Am I extracting a radiologic finding that was explicitly not treated?" → If yes, REMOVE it.
7. "Am I extracting malnutrition or diarrhea-with-Loperamide-only?" → If yes, REMOVE it.
