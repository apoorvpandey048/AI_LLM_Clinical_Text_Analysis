SNAP-AI Layer 2: CIE – Complication Inference Engine (v1.15 — Episode-Aware, Severity-Guarded, Few-Shot Enhanced)
...
• Nutritional assessments (e.g., "Mangelernährung", "significant energy and protein malnutrition")
  ...
  Dietary support/nutritional counseling alone does NOT qualify.
• Radiologic findings WITHOUT clinical consequence: if the text explicitly states that
  NO treatment was given because parameters were regressive/stable, the finding is an
  observation, NOT a deviation. Example: "lung consolidations noted, but because infection
  parameters were regressive, no antibiotic therapy was initiated" → NOT a complication.
• Incidental lab values without clinical action
• Diarrhea managed ONLY with Loperamide without additional workup or escalation →
  routine symptom management with an exempt drug, NOT a separate complication episode
...
EXAMPLE R — Hyponatremia grading (v1.15 — CORRECTED)
  
  RULE: Hyponatremia grading depends on whether a NON-EXEMPT treatment was given:
  - Fluid restriction + diuretics ONLY → Grade I
  - Tolvaptan, Conivaptan, hypertonic saline → Grade II
  - Albumin given ANYWHERE during the same admission → Grade II
    (Albumin is a non-exempt blood product. If the patient has hyponatremia AND
     albumin was administered during the admission for ANY reason, the hyponatremia
     episode is Grade II.)

  BINDING EXAMPLES:
  Text: "Hyponatriämie, Trinkmengenrestriktion 1 L/Tag, Natrium-Kontrollen"
  → Grade I (fluid restriction + monitoring only = conservative = Grade I)

  Text: "Hyponatriämie, Therapie mit Tolvaptan"
  → Grade II (Tolvaptan = specific non-exempt drug)

  Text: "Hypervoläme Hyponatriämie. Elsewhere in admission: Albumin-Substitution."
  → Grade II (albumin = non-exempt blood product → Grade II even if albumin
     was given for a different indication like capillary leak or ascites)

============================================================
INTERVENTIONAL PROCEDURE SCAN (v1.15 — MANDATORY)

Before finalising episodes, scan the ENTIRE text for evidence of interventional procedures:
  - "punktiert" / "Punktion" / "puncture" → likely Grade IIIa
  - "Drainage" (NEW placement, NOT routine removal) → likely Grade IIIa
  - "Aszites ... punktiert" / "ascites ... drained" / "Aszitesdrainage" → Grade IIIa
  - "Pleuraerguss ... punktiert" / "Pleurapunktion" → Grade IIIa
  - "CT-gesteuerte Drainage" / "CT-guided drainage" → Grade IIIa
  - "Reoperation" / "Revision" → Grade IIIb
If found AND not already captured as an episode → ADD it. Do NOT miss interventional procedures.
...
3. If you extracted more than 4 episodes, double-check each one:
   ...
   - Is it an ASSESSMENT FINDING (malnutrition, radiologic observation) without treatment?
...
Before outputting, answer these questions internally:
1. "Did I extract any radiologic finding that was explicitly not treated?" → If yes, REMOVE it.
2. "Did I extract malnutrition or diarrhea-with-Loperamide-only?" → If yes, REMOVE it.
3. "Did I miss any percutaneous puncture or drainage?" → If yes, ADD as IIIa.
4. "Is there a named ileus + adaptierte Darmstimulation that I graded as I?" → If yes, UPGRADE to II.
