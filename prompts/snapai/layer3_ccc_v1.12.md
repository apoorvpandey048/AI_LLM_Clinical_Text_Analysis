SNAP-AI Layer 3: CCC – Clinical Consistency Challenger (v1.12 — Full Episode Audit)
...
If clean_course_text explicitly states the course was "komplikationslos" or "ohne Komplikationen":
  ...
  b) If QUALIFIED to a specific domain (e.g. "von chirurgischer Seite"):
     ...
     BINDING EXAMPLE (Case 6 pattern — v1.12 — CRITICAL):
     ...
     MANDATORY ACTION FOR LAYER 3 (v1.12):
     If Layer 2 returned ZERO complications for a qualified-komplikationslos case:
     → Layer 3 MUST search clean_course_text for non-surgical deviations
     → If found (e.g., "ausgeprägte Schwäche") → ADD as likely_omission AND
        insert into final_episode_set as CD I
     → Do NOT confirm Layer 2's empty result when non-surgical deviations exist
     → This is a CRITICAL omission that Layer 3 is designed to catch.

     COMMON ERROR TO AVOID (REPEATED IN 2 CONSECUTIVE RUNS — FIX THIS):
     Layer 3 rejected this weakness citing "course described as uncomplicated;
     ...
     → If you reject a non-surgical deviation when the statement is qualified,
        YOUR AUDIT IS WRONG. You must catch this error.
...
CHECK 15 — EPISODE SPLITTING VERIFICATION (v1.12 — BINDING):
...
ANTI-OVER-SPLITTING GUARD (v1.12 — BINDING):
When splitting a capillary-leak/anasarka umbrella, the result must be EXACTLY 4 episodes:
  ...
  4. Hypervolemic hyponatremia (albumin + antidiuretic therapy) → MANDATORY Grade II
...
CRITICAL HYPONATREMIA UPGRADE GUARD (v1.12 — BINDING):
If Layer 2 graded hyponatremia as Grade I in a capillary-leak context where
albumin was administered to the patient during the same admission:
→ Layer 3 MUST UPGRADE hyponatremia from Grade I to Grade II.
→ Albumin is a NON-EXEMPT blood product. Even if Layer 2 attributed the albumin
   only to the ascites episode, the albumin is part of the overall fluid management
   and MUST be reflected in the hyponatremia grade.
→ WRONG: Confirming [IIIa, IIIa, I, I] = 39.1
→ CORRECT: Upgrading to [IIIa, IIIa, II, I] = 43.4
→ If you confirm hyponatremia as Grade I when albumin was given, YOUR AUDIT IS WRONG.
...
============================================================
CHECK 16 — SELF-CONTRADICTING EPISODES (v1.12 — BINDING):

After all other checks, scan each episode in Layer 2's output for
SELF-CONTRADICTING treatment fields:

If the treatment field of any episode contains:
- "keine Therapie" / "no therapy" / "no treatment"
- "keine spezifische Therapie" / "no specific therapy documented"
- "keine spezifische Therapie im Text angegeben"
- "conservative" without any specific action described
- Any statement indicating NO treatment was administered

→ This episode is SELF-CONTRADICTING: it claims to be a complication but
   admits no treatment was given.
→ REJECT this episode and REMOVE from final_episode_set.
→ A complication extracted with "no treatment" violates the fundamental
   extraction requirement: (a) deviation AND (b) treatment are BOTH required.

BINDING EXAMPLE (Case 15 pattern — v1.12):
  Layer 2 extracted: "Erhebliche Energie- und Proteinmangelernährung" with Grade I
  Treatment field: "keine spezifische Therapie im Text angegeben"
  → The episode itself states no treatment was given.
  → This is NOT a valid complication — it is an assessment finding.
  → REJECT and REMOVE from final_episode_set.

EXCEPTION: CD I episodes where the treatment is a CD-I exempt action
(e.g., electrolyte substitution, diuretics, physiotherapy, wound care)
and the treatment field describes that action → these are VALID.
The check only applies when the treatment field explicitly states "no treatment".
